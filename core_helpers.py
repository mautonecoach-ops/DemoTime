...
def blocker_decision(metrics, thresholds):
    fails = []
    if metrics.get("trust", 0) < thresholds["min_trust"]:
        fails.append(("trust", metrics.get("trust", 0), thresholds["min_trust"]))
    if metrics.get("cohesion", 0) < thresholds["min_cohesion"]:
        fails.append(
            ("cohesion", metrics.get("cohesion", 0), thresholds["min_cohesion"])
        )
    if metrics.get("equity", 0) < thresholds["min_equity"]:
        fails.append(("equity", metrics.get("equity", 0), thresholds["min_equity"]))
    return fails
