...
from models import DemoEntry, Counter
import logging
...
def index():
    ...
    categories = db.session.query(DemoEntry.category, db.func.count(DemoEntry.id)).group_by(DemoEntry.category).all()
...
def forms_demo():
    ...
    recent_entries = DemoEntry.query.order_by(DemoEntry.created_at.desc()).limit(5).all()
...
def api_stats():
    ...
    try:
        ...
        categories = db.session.query(
            DemoEntry.category, 
            db.func.count(DemoEntry.id)
        ).group_by(DemoEntry.category).all()
